...
type SelectedWork = { title: string; type: string; date: string; image: string; href: string; text: string; result: string; tags: string[] };

const selectedWorksContent: Record<Lang, SelectedWork[]> = {
  ru: [
  {
    "title": "VitaPulse",
  ...
  },
  {
    ...
    "href": "https://react-pizza-xi-nine.vercel.app",
...
function SelectedWorksSection({ lang }: { lang: Lang }) {
  const isRu = lang === 'ru';
  const works = selectedWorksContent[lang];

  return (
    <section id="works" className="mx-auto max-w-7xl px-5 py-20 sm:px-6 lg:px-8">
      <div className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          <SectionLabel>{isRu ? 'Избранные работы' : 'Selected works'}</SectionLabel>
          <h2 className="mt-5 text-4xl font-black tracking-tight text-white sm:text-5xl lg:text-6xl">
            {isRu ? 'Работы, которые показывают стиль, структуру и результат.' : 'Work that shows style, structure and result.'}
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-mist">
            {isRu ? 'Короткая витрина реальных проектов и интерфейсов. Блок стоит перед FAQ и контактами, а футер остаётся последним экраном.' : 'A compact showcase of real projects and interfaces. This block stays before FAQ and contact, while the footer remains the final section.'}
          </p>
        </div>
        <a href="https://portfolio-pi-snowy-53.vercel.app/#/projects" target="_blank" rel="noreferrer" className="inline-flex w-fit items-center justify-center rounded-2xl border border-[#58d2cf]/35 bg-white/[0.04] px-7 py-4 text-sm font-black text-white transition duration-300 hover:border-[#58d2cf]/60 hover:bg-[#58d2cf]/10">
        ...
        {works.map((work, index) => (
          <article key={work.title} className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.035] p-4 shadow-[0_24px_80px_rgba(0,0,0,.22)] transition duration-500 hover:-translate-y-1 hover:border-[#58d2cf]/35 hover:bg-white/[0.055]">
            <div className="relative aspect-[16/10] overflow-hidden rounded-[1.45rem] border border-white/10 bg-[#080b10]">
              <img src={work.image} alt={work.title} className="h-full w-full object-cover object-top opacity-95 transition duration-700 group-hover:scale-[1.045] group-hover:opacity-100" />
            ...
            <div className="px-2 pb-3 pt-6">
              ...
              <p className="mt-3 min-h-[6.4rem] text-base leading-7 text-mist">{work.text}</p>
              <div className="mt-5 flex flex-wrap gap-2">
                {work.tags.map((tag) => <span key={tag} className="rounded-full border border-white/10 bg-white/[0.045] px-3 py-1.5 text-xs font-bold text-mist">{tag}</span>)}
              </div>
              <a href={work.href} target="_blank" rel="noreferrer" className="mt-6 inline-flex w-full items-center justify-center rounded-2xl border border-[#58d2cf]/25 bg-[#58d2cf]/10 px-5 py-4 text-sm font-black text-white transition duration-300 hover:border-[#58d2cf]/55 hover:bg-[#58d2cf]/18">
